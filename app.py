        ...
        if result.get('is_threat', False):
            await say(
                blocks=[
                    {
                        "type": "section",
                        "text": {"type": "mrkdwn", "verbatim": True, "text": f"Hey there <@{message['user']}>! I flagged {result.get('url', '').replace('.', '[.]')} under the category {', '.join(result.get('threat_types', [])).title()} in your message! Please exercise caution when clicking said link!"},
                    }
                ],
